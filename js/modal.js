        ...
        notesArea.innerHTML = related.length
            ? related.map((s, i) => `
                <div class="rev-card" id="rev-${i}"
                     style="border:1px solid #334155;border-radius:8px;margin:8px 0;background:rgba(255,255,255,.05);">
                    ...
                    <div class="rev-body">
                        ...
                        ${s.impact ? `<div class="mt-6"><strong>Impact:</strong> ${esc(s.impact.toString())}</div>` : ""}
                    </div>
                </div>
            `).join("")
            : `<div class="text-muted text-sm">No reviewer notes yet.</div>`;

        // Attach collapsible behavior
        notesArea.querySelectorAll(".rev-card").forEach(card => {
            const header = card.querySelector(".rev-header");
